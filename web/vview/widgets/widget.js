...
export default class Widget extends Actor {
	...
	static fromNode(node, { allowNone = false } = {}) {
		if (node == null && allowNone) return null;
	...
	constructor({
		...
		if (container != null) {
			if (containerPosition === "replace") container.replaceWith(this.root);
			else container.insertAdjacentElement(containerPosition, this.root);
		}
	...
	visibilityChanged() {
		...
		if (this.actuallyVisible) {
			...
			if (this.visibilityAbort) this.visibilityAbort.abort();
