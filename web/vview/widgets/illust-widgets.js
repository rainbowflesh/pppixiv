...
export class GetMediaInfo extends AsyncLookup {
	constructor({
		...
		ppixiv.mediaCache.addEventListener(
			"mediamodified",
			(e) => {
				if (e.mediaId == this._id) this.refresh();
			},
			this._signal,
		);
	}
	...
	async _refreshInner() {
		...
		if (this._id != null && neededData != "mediaId") {
			...
			// OK.
			if (this._info.mediaInfo == null) await this._onrefresh(this._info);
...
export class BookmarkButtonWidget extends IllustWidget {
	...
	refreshInternal({ mediaId, mediaInfo }) {
		if (this.bookmarkCountWidget) this.bookmarkCountWidget.setMediaId(mediaId);
		...
		if (this.bookmarkType == "private") this.root.hidden = isLocal;
		...
		this.root.dataset.popup =
			mediaInfo == null
				? ""
				: !bookmarked && this.bookmarkType == "folder"
					? "Bookmark folder"
					: !bookmarked && this.bookmarkType == "private"
						? "Bookmark privately"
						: !bookmarked && this.bookmarkType == "public" && type == "folder"
							? "Bookmark folder"
							: !bookmarked && this.bookmarkType == "public"
								? "Bookmark image"
								: willDelete
									? "Remove bookmark"
	...
	clickedBookmark = async (e) => {
		...
		let tagList = null;
		if (
			this._bookmarkTagListWidget &&
			this._bookmarkTagListWidget.visibleRecursively
		)
		...
		if (this._bookmarkTagListWidget) this._bookmarkTagListWidget.deactivate();
		...
		let deleteBookmark =
			this.toggleBookmark && mediaInfo.bookmarkData?.private == privateBookmark;
		if (this.bookmarkType == "delete") deleteBookmark = true;

		if (deleteBookmark) {
			if (!mediaInfo.bookmarkData) return;

			// Confirm removing bookmarks when on mobile.
			if (ppixiv.mobile) {
				let result = await new ConfirmPrompt({ header: "Remove bookmark?" })
					.result;
				if (!result) return;
			}
			...
			if (mediaId != this._mediaId) return;
...
export class LikeButtonWidget extends IllustWidget {
	...
	async refreshInternal({ mediaId }) {
		...
		this.root.closest(".button-container").hidden =
			helpers.mediaId.isLocal(mediaId);

		let likedRecently =
			mediaId != null ? ppixiv.extraCache.getLikedRecently(mediaId) : false;
	...
	clickedLike = (e) => {
		...
		if (this._mediaId != null) Actions.likeImage(this._mediaId);
	};
}
...
export class LikeCountWidget extends IllustWidget {
	...
	async refreshInternal({ mediaId, mediaInfo }) {
		let text = "";
		if (!helpers.mediaId.isLocal(mediaId)) text = mediaInfo?.likeCount ?? "---";
		this.root.textContent = text;
	}
}
