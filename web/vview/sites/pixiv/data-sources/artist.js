...
import DataSource, {
	PaginateMediaIds,
	TagDropdownWidget,
} from "/vview/sites/data-source.js";
...
export default class DataSources_Artist extends DataSource {
	...
	async loadPageInternal(page) {
		...
		if (tag == null) {
			...
			if (this.pages == null) {
				...
				if (args.hash.get("order") == "oldest") allMediaIds.reverse();
			...
			// query parameter.
			let typeForUrl =
				type == null ? "illustmanga" : type == "illust" ? "illusts" : "manga";
	...
	async loadAllResults() {
		...
		if (type == "artworks" || type == "illustrations")
			for (let illustId in result.body.illusts) illustIds.push(illustId);
		if (type == "artworks" || type == "manga")
			for (let illustId in result.body.manga) illustIds.push(illustId);
	...
	get currentTag() {
		...
		if (tag == null) tag = args.query.get("tag");
	...
	get uiInfo() {
		...
		return {
			...
			mobileTitle: this.userInfo?.name
				? `Artist: ${this.userInfo?.name}`
				: `Artist`,
	...
	async tagListOpened() {
		...
		for (let tag of this.postTags) tag.recent = userTagSearch.has(tag.tag);

		// Move tags that this artist uses to the top if the user has searched for them recently.
		this.postTags.sort((lhs, rhs) => {
			if (rhs.recent != lhs.recent) return rhs.recent - lhs.recent;
			else return rhs.cnt - lhs.cnt;
		});

		let tags = [];
		for (let tagInfo of this.postTags) tags.push(tagInfo.tag);
		this.translatedTags = await ppixiv.tagTranslations.getTranslations(
			tags,
			"en",
		);
	...
	async getUserTags(userInfo) {
		if (userInfo.frequentTags) return Array.from(userInfo.frequentTags);
		...
		for (let tagInfo of result.body) {
			if (tagInfo.tag_translation == "") continue;
	...
	getDisplayingText() {
		if (this.userInfo) return this.userInfo.name + "'s Illustrations";
		else return "Illustrations";
	}
}
	...
	constructor({ dataSource, ...options }) {
		...
		class TagDropdown extends TagDropdownWidget {
			refreshTags() {
				...
				if (dataSource.postTags != null) {
					...
					if (currentTag != null) this.addTagLink({ tag: currentTag });
				}
			}
			...
			addTagLink(tagInfo) {
				...
				if (tagInfo.tag != "All" && tagInfo.cnt < 5 && !tagInfo.recent) return;
				...
				// in tagListOpened.
				if (tagInfo.recent) classes.push("recent");
				...
				dataSource.setItem(a, {
					urlFormat,
					fields: { "/tag": tag != "All" ? tag : null },
				});

				if (tag == "All") a.dataset["default"] = 1;
		...
		this.tagDropdown = new DropdownMenuOpener({
			...
			createDropdown: ({ ...options }) =>
				new TagDropdown({ dataSource, ...options }),
